...
export function RatingsList({
  ...
  if (ratings.length === 0) {
    return <div className="text-amber-400 text-center py-4">No ratings yet</div>
  }
  ...
  return (
    <div className="space-y-3">
      {sortedRatings.map(rating => {
        ...
        const displayRating =
          isHovered && hoveredStarValue !== null
            ? hoveredStarValue
            : rating.rating
        ...
        return (
          <div
            key={rating.id}
            className={`bg-zinc-900 p-3 md:p-4 rounded-lg border shadow-sm ${
              isCurrentUser
              ...
              <div className="flex-grow min-w-0">
                <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-2">
                  <span className="text-amber-200 font-medium truncate">
                    {rating.user.name}
                    {isCurrentUser && (
                      <span className="text-amber-600 text-xs ml-1">(You)</span>
                    )}
                  </span>
                        ...
                        <div key={index} className="relative">
                          {isHalfStar ? (
                            ...
                            <StarIcon
                              size={18}
                              className={
                                isFullStar ? 'text-amber-400' : 'text-amber-600'
                              }
                      ...
                      if (isCurrentUser && onRatingChange) {
                        return (
                          <button
                            ...
                            type="button"
                            onClick={e =>
                              handleStarClick(rating.id, starValue, e)
                            }
                            onMouseMove={e =>
                              handleStarHover(rating.id, starValue, e)
                            }
                            onMouseLeave={() => {
