...
const StatusList: React.FC<StatusListProps> = ({
  ...
  if (sortedStatuses.length === 0) {
    return <p className="text-amber-500 text-sm">No reading statuses yet.</p>
  }
    ...
    <div className="space-y-3">
      {sortedStatuses.map(status => {
        ...
        return (
          <div
            key={status.id}
            className={`flex items-start md:items-center justify-between p-3 rounded-lg gap-3 ${
              isCurrentUser
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-2">
                  ...
                  {isCurrentUser && onStatusChange ? (
                    ...
                    <span className="px-3 py-1 bg-zinc-700/50 border border-amber-900/30 rounded-md text-sm text-amber-200 inline-block">
                      {
                        ReadingStatusLabels[
                          status.status_id as ReadingStatusValue
                        ]
                      }
                    </span>
                  )}
