...
export function EditCategoryModal<T extends CategoryItem>({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : `Failed to update ${categoryType}`)
        }
    }
                        ...
                        <div className="flex flex-wrap gap-2 mb-3">
                            {colorOptions.map(colorOption => (
                                <button
                                    ...
                                    className={`w-8 h-8 rounded-full border-3 transition-all hover:scale-110 ${color === colorOption
